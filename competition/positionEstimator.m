...
if length(test_data.spikes) <= 320
    ...
    for j=1:20/bin_size
        ...
        %     numspikes = length(find(test_data.spikes(i,tmin:tmax)==1));
            numspikes= sum(test_data.spikes(i, tmin:(tmax-(20/bin_size-j)*bin_size)));
            firingRate(i) = numspikes/(bin_size*0.001);
        end
    ...
    % s = s_0 + v * t
    x = x + velocity_x*(bin_size*0.001);
    y = y + velocity_y*(bin_size*0.001);
end
